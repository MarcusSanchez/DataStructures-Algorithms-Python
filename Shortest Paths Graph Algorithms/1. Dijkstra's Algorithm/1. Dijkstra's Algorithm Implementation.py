...
class Node:
    ...
    def __init__(self, name):
        self.name = name
        self.visited = False
        # this is the node we came from in the shortest path
...
class DijkstraAlgorithm:
    ...
    def calculate(self, start_vertex):
        ...
        # have to iterate until heap is not empty
        while self.heap:

            # we pop the vertex with the lowest min_distance
            # the pop function removes the given item !!!
            ...
            for edge in actual_vertex.adjacency_list:
                ...
                # we have to compare the min distances
                new_distance = u.min_distance + edge.weight

                # there is a shorter path to the v vertex
                if new_distance < v.min_distance:
                    # when there is a shorter path available then we update the
